...
class MachineToolsLister:
    ...
    def by_type(self, machine_type: confloat(ge=0, le=9)) -> list:
        ...
        return [
            ...
            for row in self._finder.by_type(machine_type=machine_type)
        ]
    ...
    def by_group(self, group: confloat(ge=0, le=9)) -> list:
        ...
        return [
            self._creator.by_name(row["Станок"])
            for row in self._finder.by_group(group=group)
        ]

    @output_debug_message("Создание списка доступных станков по классу {}")
    def by_type_and_group(
        self, machine_type: confloat(ge=0, le=9), group: confloat(ge=0, le=9)
    ) -> list:
        self._finder._verbose = True
        return [
            self._creator.by_name(row["Станок"])
            for row in self._finder.by_type_and_group(
                machine_type=machine_type, machine_group=group
            )
        ]
