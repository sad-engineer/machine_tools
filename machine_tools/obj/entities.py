...
)  # для сохранения данных с ошибкой
...
class MachineTool(BaseModel):
    ...
    performance_proc: Optional[confloat(ge=0, le=1)] = Field(
        None, description="КПД станка"
    )
    power_lathe_passport_kvt: Optional[PositiveFloat] = Field(
        None, description="Мощность главного двигателя"
    )
    ...
    automation: Optional[InTypesOfAutomation] = Field(
        None, description="Класс автоматизации"
    )
    accuracy: Optional[str] = Field(None, description="Класс точности")
    specialization: Optional[InTypesOfSpecialization] = Field(
        None, description="Класс специализации"
    )
