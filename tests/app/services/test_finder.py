...
class TestMachineFinder(unittest.TestCase):
            ...
            Machine(
                ...
                technical_requirements=[
                    TechnicalRequirement(
                        id=5,
                        machine_name="2А135",
                        requirement="max_drill_diameter",
                        value=35
                    ),
                    TechnicalRequirement(
                        id=6,
    ...
    def test_01_by_power(self):
        ...
        self.finder.by_power(min_power=7.0)
        ...
        self.finder.by_power(max_power=6.0)
        # Проверяем, что builder вызван с правильными параметрами
        ...
        self.finder.by_power(order_by_power=True, descending=True)
        # Проверяем, что builder вызван с правильными параметрами
    ...
    def test_02_by_efficiency(self):
        ...
        self.finder.by_efficiency(min_efficiency=0.8)
        # Проверяем, что builder вызван с правильными параметрами
        self.mock_builder.filter_by_efficiency.assert_called_once_with(min_efficiency=0.8, max_efficiency=None)
        self.mock_builder.execute.assert_called_once()

        # Сортировка по КПД
        self.finder.by_efficiency(order_by_efficiency=True, descending=True)
        # Проверяем, что builder вызван с правильными параметрами
    ...
    def test_05_by_specialization(self):
        ...
        self.finder.by_specialization("Токарная обработка")
        # Проверяем, что builder вызван с правильными параметрами
    ...
    def test_06_by_name(self):
        ...
        self.finder.by_name("16К20")
        # Проверяем, что builder вызван с правильными параметрами
        self.mock_builder.filter_by_name.assert_called_once_with("16К20", case_sensitive=False, exact_match=True)
        self.mock_builder.execute.assert_called_once()

        # Поиск по вхождению строки
        self.finder.by_name("16К20", exact_match=False)
        # Проверяем, что builder вызван с правильными параметрами
        self.mock_builder.filter_by_name.assert_called_with("16К20", case_sensitive=False, exact_match=False)
        self.mock_builder.execute.assert_called()

        # Поиск с учетом регистра
        self.finder.by_name("16к20", case_sensitive=True)
        # Проверяем, что builder вызван с правильными параметрами
        self.mock_builder.filter_by_name.assert_called_with("16к20", case_sensitive=True, exact_match=True)
        self.mock_builder.execute.assert_called()

        # Поиск по вхождению с учетом регистра
        self.finder.by_name("16к20", case_sensitive=True, exact_match=False)
        # Проверяем, что builder вызван с правильными параметрами
        self.mock_builder.filter_by_name.assert_called_with("16к20", case_sensitive=True, exact_match=False)
        self.mock_builder.execute.assert_called()
